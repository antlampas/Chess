...
std::vector<std::string> board::listValidMoves(std::string coordinates)
{
    ...
    if(decodedCoordinates.first != -1)
    {
        ...
        if(this->isPieceNameValid(piece))
        {
            ...
            {
                if((row!=0) && (row!=7))
                    if(!this->isColumnClogged(coordinates,this->encodeCoordinates(std::pair<int,int>(7,column))) && !this->isColumnClogged(this->encodeCoordinates(std::pair<int,int>(0,column)),coordinates))
                    {
                    ...
                    if(!this->isColumnClogged(this->encodeCoordinates(std::pair<int,int>(0,column)),coordinates))
                        for(int i=row-1;i>-1;i--) movesList.push_back(this->encodeCoordinates(std::pair<int,int>(i,column)));
                if((column!=0) && (column!=7))
                    if(!this->isRowClogged(coordinates,this->encodeCoordinates(std::pair<int,int>(row,7))) && !this->isRowClogged(this->encodeCoordinates(std::pair<int,int>(row,0)),coordinates))
                    {
